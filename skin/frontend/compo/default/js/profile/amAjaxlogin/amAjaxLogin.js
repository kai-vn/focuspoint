...
AmAjaxLogin.prototype = 
{
    ...
    showMessage: function(response) {
        ...
        hideDiv = $(hideDiv);
        hideDiv.id = 'hideDiv';
        if (!jQuery('body').hasClass('mastercheckout-index-index')) {
            Event.observe(hideDiv, 'click', function () {
                AmAjaxLoginObj.hideMessage()
            });
        }
...
window.onload = function () {
    if (!jQuery('div').hasClass('customer-info')) {
        if (jQuery('body').hasClass('mastercheckout-index-index')) {
            AmAjaxLoginObj.sendLoginAjax();
        }
    }
};
function methodGuest(){
    jQuery('.customer-dob').css("display","none");
    jQuery("label[for='shipping:month']").css("display","none");
    if($('shipping:gender')){
        $('shipping:gender').hide();
    }
    jQuery("label[for='shipping:gender']").css("display","none");
